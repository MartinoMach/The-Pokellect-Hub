  ...
  handler: async (request, context) => {
    try {
      ...
      if (q) {
        ...
        querySpec = {
          query: "SELECT TOP 20 c.username, c.displayName, c.avatarUrl FROM c WHERE c.username != @currentUser AND (CONTAINS(LOWER(c.username), LOWER(@q)) OR (IS_DEFINED(c.displayName) AND CONTAINS(LOWER(c.displayName), LOWER(@q))))",
          parameters: [
        ...
        querySpec = {
          query: "SELECT TOP 20 c.username, c.displayName, c.avatarUrl FROM c WHERE c.username != @currentUser",
