...
const getTcgapiCardsPage = async (franchiseId, options = {}) => {
  ...
  const { page, limit, ...searchParams } = options;

  const validatedPage = Number.isFinite(page) ? Math.max(1, Math.floor(page)) : 1;
  const validatedLimit = Number.isFinite(limit) ? Math.max(1, Math.min(100, Math.floor(limit))) : 25;
  ...
  return {
    page: Number(data?.page) || validatedPage,
    limit: Number(data?.limit) || validatedLimit,
