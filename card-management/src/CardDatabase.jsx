...
import { addToBinder, importTcgapiCard, searchTcgapi, getMetadata } from './api.js';
...
function CardDatabase() {
  ...
  const { globalCards, isLoading, refreshBinder, loadData } = useData();
  ...
  // This function now handles adding from BOTH search and manual ID entry
  const handleImportCardToDB = async () => {
    const cardToImportId = selectedApiCard?.id || manualCardId;
    if (!cardToImportId || !addCardFranchise) {
      setAddCardMessage('Please select a card or enter an ID.');
      return;
    }
    ...
    try {
      ...
      await loadData(); // Refresh all data to get the new card
    ...
    <div className="dashboard-container">
        ...
        <div className="cards-grid">
          {isLoading ? (
            ...
            filteredCards.map((card, idx) => (
              <div key={card.id || idx} className="collection-card" onClick={() => setSelectedStatsCard(card)}>
                <img
                ...
                />
                <h4>{card.name}</h4>
                <span className="card-franchise-badge">{formatLabel(card.franchiseId)}</span>
              </div>
            ))
          )}
        </div>
        ...
        {selectedStatsCard && (
          <div className="dialog-content stats-content">
            ...
            <div className="stats-details">
              <p><strong>Franchise:</strong> {formatLabel(selectedStatsCard.franchiseId)}</p>
              {(selectedStatsCard.setName || selectedStatsCard.setId) && <p><strong>Set Name:</strong> {selectedStatsCard.setName || selectedStatsCard.setId}</p>}
              {selectedStatsCard.rarity && <p><strong>Rarity:</strong> {selectedStatsCard.rarity}</p>}
              {selectedStatsCard.types && selectedStatsCard.types.length > 0 && <p><strong>Type:</strong> {Array.isArray(selectedStatsCard.types) ? selectedStatsCard.types.join(', ') : selectedStatsCard.types}</p>}
              {selectedStatsCard.hp && <p><strong>HP:</strong> {selectedStatsCard.hp}</p>}
              {selectedStatsCard.power && <p><strong>Power:</strong> {selectedStatsCard.power}</p>}
              {selectedStatsCard.currentPrice != null && <p><strong>Market Price:</strong> ${Number(selectedStatsCard.currentPrice).toFixed(2)}</p>}
            </div>
      ...
      <dialog ref={addCardDialogRef} className="glass-dialog" onClose={() => setAddCardMessage('')}>
        <div className="dialog-content">
          ...
          {/* Main Content: Either Search flow or Advanced ID flow */}
          {showAdvancedAdd ? (
            <div className="register-form">
              <div className="filter-group">
                <label>Franchise</label>
                <select className="filter-select" value={addCardFranchise} onChange={(e) => setAddCardFranchise(e.target.value)}>
              ...
              <div className="filter-group">
                <label>Card ID</label>
                <input type="text" placeholder="e.g. base1-4" className="filter-input" value={manualCardId} onChange={(e) => setManualCardId(e.target.value)} />
              </div>
            </div>
          ) : (
            <>
              <div className="register-form">
                ...
                <div className="filter-group">
                  <label>Search by Name</label>
                  <input type="text" placeholder="e.g. Charizard, Son Goku..." className="dialog-input" value={apiSearchQuery} onChange={(e) => { setApiSearchQuery(e.target.value); setSelectedApiCard(null); }} />
                </div>
              </div>
          ...
          <div className="dialog-actions">
            <a className="advanced-toggle-link" onClick={() => setShowAdvancedAdd(!showAdvancedAdd)}>
              {showAdvancedAdd ? '← Search by Name' : 'Add by ID (Advanced)'}
            </a>
            ...
            <button
              ...
              disabled={isAddingToDB || (!selectedApiCard && !manualCardId)}
            >
